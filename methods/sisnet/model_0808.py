...
class info_block_orig(nn.Module):
    def __init__(self, config, feat):
        super(info_block_orig, self).__init__()
        self.res_conv1 = nn.Conv2d(feat, feat, 3, padding=1)
        self.fuse = nn.Conv2d(feat * 2, feat, 3, padding=1)
    ...
    def forward(self, x0, x1, fdm):
        ...
        x0 = self.fuse(torch.cat([x0 * res_x1, x1], dim=1))
        sup = self.sup(x0)
...
class info_block(nn.Module):
    def __init__(self, config, feat):
        ...
        self.inner_conv = nn.Sequential(*list(up_conv(feat, feat, False)))
        self.boun_conv = nn.Sequential(*list(up_conv(feat, feat, False)))
        self.bsup = nn.Conv2d(feat, 1, 1)
        
        self.fuse_conv_0 = nn.Sequential(*list(up_conv(feat * 2, feat, False)))
        self.fuse_conv_1 = nn.Sequential(*list(up_conv(feat * 2, feat, False)))
    ...
    def forward(self, x0, x1, fdm):
        x0 = self.inner_conv(x0)
        x0b = self.boun_conv(x0)
        bup = self.bsup(x0b)
        
        x0 = self.fuse_conv_0(torch.cat([x0, x0b], dim=1))
        sup = self.sup(x0)
        x0 = nn.functional.interpolate(x0, size=x1.size()[2:], mode='bilinear')
        
        x1 = self.fuse_conv_1(torch.cat([x0, x1], dim=1))
        #x1 = nn.functional.interpolate(x1, size=fdm.size()[2:], mode='bilinear')
...
class decoder(nn.Module):
    def __init__(self, config, encoder, feat):
        ...
        self.info0 = info_block_orig(config, num_channel)
        self.info1 = info_block_orig(config, num_channel)
        self.info2 = info_block_orig(config, num_channel)
        self.info3 = info_block_orig(config, num_channel)
        self.info4 = info_block_orig(config, num_channel)
